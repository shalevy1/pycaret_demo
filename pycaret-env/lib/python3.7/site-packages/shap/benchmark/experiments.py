...
def get_experiments(dataset=None, model=None, method=None, metric=None):
...
def run_experiments(dataset=None, model=None, method=None, metric=None, cache_dir="/tmp", nworkers=1):
    experiments_arr = list(get_experiments(dataset=dataset, model=model, method=method, metric=metric))
    if nworkers == 1:
...
def run_remote_experiments(experiments, thread_hosts, rate_limit=10):
    ...
    experiments : iterable
        Output of shap.benchmark.get_experiments(...).
